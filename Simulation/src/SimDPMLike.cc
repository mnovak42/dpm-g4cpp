...
Geom Simulate(int nprimary, double eprimary, bool iselectron, double lbox, SimMaterialData& matData,
              SimElectronData& elData, SimPhotonData& phData, int geomIndex) {
    ...
    for (int iPrimary = 0; iPrimary < nprimary; ++iPrimary) {
        ...
        while (-1 < TrackStack::Instance().PopIntoThisTrack(track)) {
            ...
            while (track.fEkin > 0.0) {
                ...
                int whatHappend = KeepTrackingElectron(elData, matData, geom, track, numTr1MFP, numMollerMFP,
...
int KeepTrackingElectron(SimElectronData& elData, SimMaterialData& matData, Geom& geom, Track& track, double& numTr1MFP,
                         double& numMollerMFP, double invMollerMFP, double& numBremMFP) {
    int whatHappend = 0;
    ...
    while (whatHappend == 0) {
        ...
        double delNumTr1MFP =
            elData.GetITr1MFPElastic()->GetITr1MFPPerDensity(midStepE, theVoxelMatIndx) * theVoxelMatDensity;
        ...
        if (stepElastic < stepLength) {
            ...
            stepElastic = numTr1MFP / (elData.GetITr1MFPElastic()->GetITr1MFPPerDensity(track.fEkin, theVoxelMatIndx) *
                                       theVoxelMatDensity);
...
void KeepTrackingPhoton(SimPhotonData& phData, SimMaterialData& matData, Geom& geom, Track& track) {
    ...
    while (track.fEkin > 0.0) {
        ...
        theProb += phData.GetIMFPCompton()->GetIMFPPerDensity(track.fEkin, theVoxelMatIndx) * theVoxelMatDensity *
                   globalMaxMFP;
        if (r1 < theProb) {
        ...
        theProb += phData.GetIMFPPairProd()->GetIMFPPerDensity(track.fEkin, theVoxelMatIndx) * theVoxelMatDensity *
                   globalMaxMFP;
        if (r1 < theProb) {
...
void PerformBrem(Track& track, SimSBTables* theSBTable) {
    ...
    const double eGamma = theSBTable->SampleEnergyTransfer(track.fEkin, track.fMatIndx, Random::UniformRand(),
                                                           Random::UniformRand(), Random::UniformRand());
...
void PerformMoller(Track& track, SimMollerTables* theMollerTable) {
    ...
    const double secEkin = theMollerTable->SampleEnergyTransfer(track.fEkin, Random::UniformRand(),
                                                                Random::UniformRand(), Random::UniformRand());
...
void PerformMSCAngularDeflection(Track& track, double ekin0, SimGSTables* theGSTables) {
    const double kPI  = 3.1415926535897932;
    const double dum0 = theGSTables->SampleAngularDeflection(ekin0, Random::UniformRand(), Random::UniformRand());
...
void PerformCompton(Track& track, SimKNTables* theKNTables, SimMaterialData& matData, Geom& geom) {
    ...
    const double theEps = theKNTables->SampleEnergyTransfer(track.fEkin, Random::UniformRand(), Random::UniformRand(),
                                                            Random::UniformRand());
